...
const TicketDashboard = () => {
  ...
  useEffect(() => {
    const fetchEngineers = async () => {
      try {
        const res = await axios.get("/api/v1/admin/dashboard/all-engineers");
        setEngineers(res.data.engineers);
      } catch (error) {
        console.error("Error fetching engineers:", error);
        toast.error(error.response?.data?.message || "Failed to fetch engineers");
      }
    };
    fetchEngineers();
  }, []);
        ...
        <div className="card p-6">
          <div className="flex items-center">
            ...
            <div className="ml-5">
              ...
              <p className="text-2xl font-semibold text-gray-900">
                {tickets?.filter(ticket => ticket.status === "not_resolved").length}
              </p>
            </div>
          </div>
        </div>
                    ...
                    recentTickets?.map(ticket => (
                      <tr key={ticket._id} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <Link to={`/tickets/${ticket._id }`} className="text-[#0F52BA]/70 hover:text-[#0F52BA]/90">
                            {ticket?.title}
                          </Link>
                        </td>
              ...
              {engineers.length > 0 ? (
                  engineers.map(engineer => {
                    const assignedTickets = tickets.filter(
                      (ticket) => ticket.assignedTo?._id === engineer._id
                    );
                    
                    return (
                      <li key={engineer._id} className="py-4 first:pt-0 last:pb-0">
                        <div className="flex items-center justify-between">
                          <div>
                            <p className="text-sm font-medium text-gray-900">{engineer.name}</p>
                            <p className="text-xs font-medium text-gray-500">{engineer.specialization || 'General Support'}</p>
                          </div>
